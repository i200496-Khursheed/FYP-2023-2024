...
const HadithQueryBuilder = () => {
  ...
  const handleAddNarrator = () => {
    ...
    setData({
      ...data,
      narrators: [...data.narrators, { theme: '', name: '', identifier: newIdentifier }],
    });
    setNarratorLogic([...narratorLogic, 'AND']);
  };

  const handleRemoveNarrator = (identifier) => {
    const updatedNarrators = data.narrators.filter((narrator) => narrator.identifier !== identifier);
    setData({
      ...data,
      narrators: updatedNarrators,
    });
  ...
  const SendDataToBackend = () => {
    ...
    if (
      data.theme === '' &&
      data.hadith_number === '' &&
      data.narrators.every((narrator) => narrator.theme === '' && narrator.name === '') &&
      data.mentions === ''
    ) {
  ...
  useEffect(() => {
    // Fetching hadith number options
  ...
  useEffect(() => {
    // Fetching narrator name options
          ...
          <div className="add-narrator-button">
            <div className="add-content" onClick={handleAddNarrator}>
              <img
                ...
                className="add-image"
              />
              <p id="add-narrator-text">Add Condition</p>
            </div>
        </div>
        ...
        <div className="narrators">
          {data.narrators.map((narrator, index) => (
            <div key={narrator.identifier} className="narrator">
              ...
              <div className="dropdown">
                <label htmlFor={`narrator_theme_${index}`}>Hadith Theme</label>
                <Select
                  options={themeOptions}
                  isSearchable={true}
                  onChange={(selectedOption) =>
                    handleNarratorChange(index, 'theme', selectedOption.value)
                  }
                />
              </div>
              <div className="dropdown">
